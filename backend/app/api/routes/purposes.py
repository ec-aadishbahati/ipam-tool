...
async def list_purposes(db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
    res = await db.execute(select(Purpose).order_by(Purpose.name))
...
async def create_purpose(payload: PurposeCreate, db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
    ...
    obj = Purpose(name=payload.name, description=payload.description, category=payload.category, category_id=payload.category_id)
    db.add(obj)
    await db.commit()
...
async def update_purpose(purpose_id: int, payload: PurposeUpdate, db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
    res = await db.execute(select(Purpose).where(Purpose.id == purpose_id))
    obj = res.scalar_one_or_none()
    if not obj:
        raise HTTPException(status_code=404, detail="Not found")
    before = {"id": obj.id, "name": obj.name, "description": obj.description, "category": obj.category, "category_id": obj.category_id}
    ...
    await db.commit()
    await db.refresh(obj)
    after = {"id": obj.id, "name": obj.name, "description": obj.description, "category": obj.category, "category_id": obj.category_id}
