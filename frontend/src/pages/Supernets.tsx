    ...
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <div>
          ...
          {/* Expanded subnets display */}
          {(data ?? []).map((s: any) => (
            expandedSupernets.has(s.id) && s.subnets && s.subnets.length > 0 && (
              <div key={`expanded-${s.id}`} className="mt-2 bg-gray-50 p-3 rounded border">
                <h4 className="text-sm font-medium mb-2">Allocated Subnets for {s.name}:</h4>
